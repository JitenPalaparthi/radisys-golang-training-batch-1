...
func main() {
	...
	done := make(chan struct{}) // there is no space to send a value
	// still it is valid becase
	go func() {
		ch <- 100
	}()
	go func() {
	...
	<-done // main is blocked here until value is sent to done channel as above func
	//none := <-done
	//fmt.Println(none)
	// add(10, 20) // add returns a value but there is no variable to get the value..
	// it is valid from the compiler perspective but it does not make any sense from the functional perspective
	// can also do the same with channels
}
